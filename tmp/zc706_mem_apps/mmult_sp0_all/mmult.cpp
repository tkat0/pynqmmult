...
int main(int argc, char* argv[]){
  int test_passed = 0;
  float *tin1Buf = NULL, *tin2Buf = NULL, *toutBufSw, *toutBufHw=NULL;
  
  printf("Testing with A_NROWS = A_NCOLS = B_NCOLS = B_NROWS = %d\n", A_NROWS);

  tin1Buf = (float *)sds_mmap((void *)(0x80000000), A_NROWS * B_NCOLS * sizeof(float), tin1Buf);
  tin2Buf = (float *)sds_mmap((void *)(0x80010000), A_NROWS * B_NCOLS * sizeof(float), tin2Buf);
  toutBufHw = (float *)sds_mmap((void *)(0x80020000), A_NROWS * B_NCOLS * sizeof(float), toutBufHw);
  toutBufSw = (float *)sds_alloc(A_NROWS * B_NCOLS * sizeof(float));

  if (!tin1Buf || !tin2Buf || !toutBufHw || !toutBufSw) {
    if (tin1Buf) sds_munmap(tin1Buf);
    if (tin2Buf) sds_munmap(tin2Buf);
